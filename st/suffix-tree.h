...
class SuffixTree
{
  ...
  private:
	...
	void List(std::u32string &u32str, int node) const
	{
		...
		for (const auto &next : tree[node].next)
		{
  ...
  public:
	...
	bool Serialize(std::filesystem::path directory, std::string filename) const
	{
		...
		for (const auto &node : tree)
		{
			...
			{
				treeFileOut.write((const char *)&pair.first, sizeof(pair.first));
				treeFileOut.write((const char *)&pair.second, sizeof(pair.second));
			}
		}
	...
	bool Contain(const std::u32string_view &u32strv) const
	{
		...
		for (int i = 0; i < u32strv.size(); ++i)
		{
			...
			{
				const auto &child = tree[curNode].next.find(u32strv[i]);
